...
syn keyword ebslKeyword ABORT ACTIVATEKEY ADD ADD_ONLY ALL ALLOW_CANCEL ALL_OF AND_EV AND_FROM APPEND ASCENDING ASSIGN ASSOCIATED ASYNC AT AUX
syn keyword ebslKeyword BATCHED BEFORE BEGIN BOTH BPIOCP BPIOCPN BREAK BREAKING_ON BROWSE BUFFER.KEYS BUT_NOT_FROM BY
syn keyword ebslKeyword CALL CALLC CALLING CALL_PROCESS CALL_SCREEN CALL_SUBR CANCEL CANCELLING CAPTURE CAPTURING CASE CDD CHAIN CHAIN_SCREEN CLEAR CLEARCOM CLEARCOMMON CLEARDATA CLEARFILE CLEARINPUT CLEARSELECT CLEARSQL CLOSE CLOSESEQ COM COMMIT COMMITTING COMMIT_EVERY COMMON COMO CONCAT CONDITIONALLY CONFIRM CONSOLIDATING CONTINUE CONVAFTER CONVERT CONV_AFTER COPY_LOGICAL COUNTED CREATE_NEW CRITERIA CRT
syn keyword ebslKeyword DATA DEACTIVATEKEY DEBUG DEFAULT_RECORD DEFER_COMMIT DEFFUN DEL DELETE DELETELIST DELETEU DELETE_RECORD DELETE_RECORDS DELETING DESCENDING DIM DIMENSION DISABLEDEC DISPLAY DO
syn keyword ebslKeyword ECHO ELSE EMAIL ENABLEDEC END ENTER EQU EQUATE ERROR ERROR_IF EXECUTE EXECUTESQL EXISTING EXIT
syn keyword ebslKeyword FIELD FIELDS FILELOCK FILEUNLOCK FIND FINDSTR FIRST FLUSH FOOTING FOR FORCE FORMLIST FROM FULL_ACCESS FUNCTION
syn keyword ebslKeyword GARBAGECOLLECT GET GETLIST GETTING_ACTION_CODES GETX GO GOSUB GOTO GRAPHED GROUPSTORE
syn keyword ebslKeyword HEADING HOLD HUSH
syn keyword ebslKeyword IF IGNORE IN INPUT INPUTCLEAR INPUTERR INPUTIF INPUTNULL INPUTTRAP INQUIRY INS INSTANCE_OF INTO INTO_REFERENCED ITEMIZING
syn keyword ebslKeyword KEY KEY_IN
syn keyword ebslKeyword LAST LEFT LENGTH LIMITING LINE LIT LITERALLY LOCATE LOCK LOCKED LOOKUP LOOP
syn keyword ebslKeyword MAT MATBUILD MATCHING MATPARSE MATREAD MATREADL MATREADU MATWRITE MATWRITEU MDPERFORM MOVE_CONTROLLER
syn keyword ebslKeyword NEW NEWLIST NEXT NOADD NOBROWSE NOCONV NOCONVERT NODELAY NOFMT NOT NO_ACCESS NO_ADD NO_CONV NO_DELETE NULL
syn keyword ebslKeyword OFF ON ONLY OPEN OPENSEQ OSBREAD OSBWRITE OSCLOSE OSDELETE OSOPEN OSREAD OSWRITE OUTPUT
syn keyword ebslKeyword PAGE PASSCOM PASSLIST PAUSE PCPERFORM PDF PERFORM PHANTOM PRECISION PRE_ASSEMBLED PRINT PRINTER PRINTERR PRINT_DETAIL PRINT_TOTAL PROCEDURE PROCREAD PROCWRITE PROGRAM PROMPT PROMPT_IN_PLACE PROMPT_WIDE PROTECTED
syn keyword ebslKeyword READ READBCK READBCKL READBCKU READFWD READFWDL READFWDU READL READLIST READNEXT READNEXTTUPLE READONLY READSELECT READSEQ READT READU READV READVL READVU READXBCK READXFWD RECORDLOCKL RECORDLOCKU RECORD_CACHING RECURSIVE REFERENCED RELEASE RELEASING REMOVE REPEAT REPROMPT RESET RESIZET RETCODE RETURN RETURNING REWIND RIGHT RNDSEED RQM RTNLIST RUN
syn keyword ebslKeyword SCREEN SECONDARY SELECT SELECTED SELECTINDEX SEND SENDX SETINDEX SETPTR SETTING SET_SECURITY SHOWA SHOWC SINGLE SLEEP SORTING START STEP STMT STOP SUBROUTINE SUSPEND SWAP SYNC
syn keyword ebslKeyword TEMPLATE TESTING THEN THIS_APPLICATION_ONLY THIS_INSTANCE_OF THROW_ERROR TO TRANSACTION TRANSLATE TRANSLATING TYPE
syn keyword ebslKeyword UNFILTERED UNIT UNLOCK UNTIL UNVALIDATED UPDATE USING UTDEXECUTE
syn keyword ebslKeyword VALIDATE VALIDATE.KEY VALIDATION
syn keyword ebslKeyword WAITING WAKE WARNING WEOF WEOFSEQ WHILE WINDOW WINDOWS WITH WRITE WRITELIST WRITEONLY WRITESEQ WRITESEQF WRITET WRITEU WRITEV WRITEVU WRITING
...
syn region ebslFuncArguments matchgroup=ebslFuncParentheses start="(" end=")" contained transparent oneline
